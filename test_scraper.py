import unittest
#import scraper
from scraper import make_params
import sys
...
class ScraperTest(unittest.TestCase):
    ...
    def test_query_only_one(self):
        actual = make_params(query="hello")
        expected = {"query": "hello"}
        self.assertEquals(actual, expected)

    def test_query_only_four(self):
        actual = make_params(query="hello world big house")
        expected = {"query": "hello world big house"}
        self.assertEquals(actual, expected)

    def test_cats(self):
        actual = make_params(pets_cat=1)
        expected = {"pets_cat": 1}
        self.assertEquals(actual, expected)
        with self.assertRaises(ValueError) as context:
            make_params(pets_cat=10)
        self.assertEqual(context.exception.message,
                         u"pets_cat can only be None or 1")

    def test_dogs(self):
        actual = make_params(pets_dog=1)
        expected = {"pets_dog": 1}
        self.assertEquals(actual, expected)
        with self.assertRaises(ValueError) as context:
            make_params(pets_dog=10)
        self.assertEqual(context.exception.message,
                         u"pets_dog can only be None or 1")

    def test_minAsk(self):
        actual = make_params(minAsk=100)
        expected = {"minAsk": 100}
        self.assertEquals(actual, expected)
    ...
    def test_bdrm_error(self):
        out = StringIO()
        sys.stdout = out
        actual = make_params(bedrooms="foo")
        a_error = out.getvalue().strip()
        ...
        out2 = StringIO()
        sys.stdout = out2
        actual2 = make_params(bedrooms=10)
    ...
    def test_bathrooms(self):
        actual = make_params(bathrooms=2)
    ...
    def test_bath_error(self):
        out = StringIO()
        sys.stdout = out
        actual = make_params(bathrooms="foo")
        a_error = out.getvalue().strip()
    ...
    def test_minSqft_error(self):
        out = StringIO()
        sys.stdout = out
        actual = make_params(minSqft="foo")
        a_error = out.getvalue().strip()
    ...
    def test_maxSqft(self):
        actual = make_params(maxSqft=100)
    ...
    def test_maxSqft_error(self):
        out = StringIO()
        sys.stdout = out
        actual = make_params(maxSqft="foo")
        a_error = out.getvalue().strip()
    ...
    def test_housing(self):
        actual = make_params(housing_type=2)
    ...
    def test_housing_error(self):
        out = StringIO()
        sys.stdout = out
        actual = make_params(housing_type="foo")
        a_error = out.getvalue().strip()
...
12")
        ...
        out2 = StringIO()
        sys.stdout = out2
        actual2 = make_params(housing_type=15)
        a_error2 = out2.getvalue().strip()
...
12")
    ...
    def test_many_urls(self):
        actual = make_params(query="back porch", maxAsk=1500,
                             minSqft=500)
